...
Первый запуск ноутбука на 12 ядрах — 7 минут 28 секунд, из них сборка 479 признаков
с нуля 26 секунд, остальное кросс-валидация. Повторный запуск короче: матрица признаков
...
| признаков | 46 | 479 |
| P@R70 | 0.7381 | **0.8938** |
| ROC-AUC | 0.9240 | 0.9529 |
| PR-AUC | 0.7699 | 0.8419 |
| recall при FPR 1 % | 0.6225 | 0.7508 |

Модель — LightGBM с gradient-based one-side sampling, обученный под 10 сидами
с усреднением в ранговом пространстве.
...
почему.
Отвергнутого там больше, чем принятого: ансамбль трёх семейств, блендинг с не-древесными
семействами, подбор гиперпараметров, отдельная модель для мобильного трафика, target
encoding, веса по свежести, псевдоразметка, граф со-просмотров, пособытийное отношение
правдоподобия, популярность по необрезанному логу, двухступенчатый реранкинг — всё
измерено на том же протоколе и откачено.

Последние три отвергнутых поучительны тем, что каждое по отдельности сильно: граф
со-просмотров даёт одиночный AUC 0.802, отношение правдоподобия — 0.860, рекорд среди
всех столбцов проекта. Прирост к набору у обоих нулевой. Модель упирается в информацию
в логе, а не в то, как эта информация ей подана.
