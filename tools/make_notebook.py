...
по `cookie_id`. Поэтому валидация трёхслойная: повторный стратифицированный K-fold на
всём train (основная, даёт максимум позитивов в OOF), forward-chaining по дням
(ловит то, что не переносится во времени) и строгий протокол `src.strict`, который
пересобирает саму матрицу признаков по дням до среза. Зачем понадобился третий — в
пункте 5.
...
**4. Что оказалось решающим.** Три места, в порядке величины по абляции на текущем
наборе.

Первое — **популярность объявления**: сколько других кук открывали то же объявление.
Удаление блока стоит −0.035 PR-AUC, вдвое больше следующего. У объявлений, которых
касался бот, 51 % аудитории — боты, при доле ботов 8 % среди кук: 899 ботов топчутся по
пулу из 6957 объявлений, а 10192 человека размазаны по 45538. Объявление с аудиторией
ровно в одну куку встречается у человека в 38 % случаев, у бота в 14 %. Вся прежняя
матрица описывала, *как* кука себя ведёт; этот блок описывает, *куда* она ходит
относительно остальных, и дал +0.086 P@R70.

Второе — **курсор**, −0.017 PR-AUC, причём не формой траектории, а самим фактом наличия
координат: на web/desktop поле заполнено у 66 % событий людей и у 33 % событий ботов.

Третье — разбор **User-Agent**: скриптовые HTTP-клиенты (curl, Scrapy, node-fetch,
python-requests, python-urllib3, Go-http-client) сваливались в общую корзину `other`
вместе с нативным приложением, то есть самое сильное положительное свидетельство
усреднялось со своей противоположностью. Разделение дало +0.012 P@R70 одним шагом.

**5. Как принимались решения.** Целевая метрика крайне шумная: на ранних моделях
размах по случайным фолдам был 0.61–0.79, по дням 0.49–0.79 при стабильном ROC-AUC
0.90–0.96, и у финальной модели разброс по дням всё ещё 0.78–0.98. Поэтому
