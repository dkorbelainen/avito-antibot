...
**3. Признаки.** Готовых поведенческих признаков нет, собрано 479 в 18 блоках:
...
перцентильные ранги внутри своей платформы, **популярность объявлений** и поведение
относительно популяции на том же контенте.

**4. Что оказалось решающим.** Три места. Leave-one-block-out показал, что курсор даёт
...
Третье и самое крупное — **популярность объявления**: сколько других кук открывали то
же объявление. У объявлений, которых касался бот, 51 % аудитории — боты, при доле ботов
8 % среди кук. 899 ботов топчутся по пулу из 6957 объявлений, 10192 человека размазаны
по 45538; объявление с аудиторией ровно в одну куку встречается у человека в 38 %
случаев, у бота в 14 %. Вся прежняя матрица описывала, *как* кука себя ведёт, этот блок
описывает, *куда* она ходит относительно остальных, — и дал +0.086 P@R70.
...
К этому гейту добавлен третий протокол, `src.strict`, и он оказался необходим.
Forward-chaining режет матрицу, построенную один раз по всему train, поэтому кука дня k
описана словарями и популяционными статистиками, видевшими дни k+1 и дальше. Для
большинства блоков это безобидно, для популяционных — нет. `src.strict` **пересобирает
матрицу заново** по дням до среза и оценивает дни после: ровно положение скрытого
теста. Первая версия блока популярности давала +0.110 P@R70 по обычному протоколу и
+0.028 по строгому — признак считался счётчиком по фиксированному пулу train, из-за чего
train-кука сравнивалась с современниками, а test-кука нет. Счётчик заменён перцентилем
внутри пула, пул сделан общим для обеих сторон, и разрыв между протоколами упал с 0.110
до 0.007.
...
Прибыль даёт не смешивание семейств, а бэггинг по сидам: среднее по сидам против их
рангового усреднения — разница примерно 0.006 PR-AUC.
""")
...
TRACK = {
    ...
    "P7_pop_pct": "+ популярность объявлений и поведение против популяции, 479",
}
...
— ранговое усреднение валидационных сидов против среднего по ним: 0.8894 → 0.8911 P@R70.

Число сидов выбрано по замеру, а не на глаз. Двадцать независимых OOF-прогонов,
смешиваемых нарастающим итогом: одиночные сиды дают P@R70 в диапазоне 0.8788…0.8930 и
PR-AUC 0.8301…0.8394, к двум сидам разброс уже снят, а дальше кривая плоская — PR-AUC
0.8410 на двух сидах, 0.8404 на пяти, 0.8400 на десяти, 0.8395 на двадцати. Двадцать
